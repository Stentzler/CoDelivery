...
const categoriesQueryBuilder = async () => {
	...
	// 	return true;
	// } else {
	// 	await AppDataSource.createQueryBuilder().delete().from(ProductCategory).execute();

	// await AppDataSource.createQueryBuilder().delete().from(RestaurantCategory).execute();
